...
**Languages:** [简体中文](README.md) | [English](README_EN.md) | **日本語**
...
中国版7.25／グローバル版7.31 以降のデータを提供しています。<br>
※ ドイツ語／フランス語はグローバル版7.41以降。<br>
※ 繁体字中国語はコミュニティのサポートが必要です。
...
5. 必要に応じて以下のコマンドを実行して抽出。
   ...
   | `npm run unpack:tc`  | 繁体字版テキストを抽出し、繁体字 CSV を `tc` に出力 |
   | `npm run unpack:ja`  | グローバル版テキスト（日本語）を `ja` に出力    |
   | `npm run unpack:en`  | グローバル版テキスト（英語）を `en` に出力    |
   | `npm run unpack:de`  | グローバル版テキスト（ドイツ語）を `de` に出力  |
   | `npm run unpack:fr`  | グローバル版テキスト（フランス語）を `fr` に出力  |
初回抽出は以上で完了します。  
...
* 言語ごとに 1 コミットにまとめる 
  ...
  > その際には2 コミットに分けること `data: GLOBAL xxx／EN` 和 `data: GLOBAL xxx／JA` 。 <br>
* コミットメッセージは下記形式を推奨
  ```
  # 中国版の例
  [タイトル] data: CHS 7.31
  [本文]    2025.09.03.0000.0000
  # グローバル版の例
  [タイトル] data: GLOBAL 7.31／JA
  [本文]    2025.09.03.0000.0000
  ```
